  ...
  return (
    <div>
      ...
      <MainContainer bg={theme?.main?.bg}>
        ...
        <BoxContainer>
          <AboutContent bg={theme?.colors.primary} >You can add tasks by clicking the icon at the bottom right corner, which opens a simple form to create a new task.</AboutContent>
          <AboutContent bg={theme?.colors.primary}>You can check the created tasks on the home page. A toast message confirms successful creation. Once a task is done, you can also update its status to mark it as complete.</AboutContent>
          <AboutContent bg={theme?.colors.primary}>You can also update or delete tasks anytime. To view all your todos, click the logo or use the sidebar on desktop. On mobile, open the menu and navigate to the All-Todos page.</AboutContent>
          ...
          <AboutContent bg={theme?.colors.primary}>The app also includes a Goals page where you can create monthly, quarterly, and yearly goals to help you improve yourself and stay focused on long-term progress.</AboutContent>
          <AboutContent bg={theme?.colors.primary}>The project also features a Streak page where you can track your progress over the last three months, helping you stay consistent and motivated.</AboutContent>
          <AboutContent bg={theme?.colors.primary}>Users can also switch between multiple themes based on their preferences, making the experience more personal. Additionally, the About page provides clear insights into the purpose and features of the project.</AboutContent>
          <AboutContent bg={theme?.colors.primary}>A standout feature of the app is the Feedback page, where users can share their thoughts and suggestions. This creates a strong connection between the user and the app, ensuring it stays up to date and continuously improves based on real feedback.
          </AboutContent>
        </BoxContainer>
